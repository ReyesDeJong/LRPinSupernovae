    ...
    def loadWeights(self, Ncnn, Nfc, path):
            ...
            for m in self.modules:
                ...
                if(Ncnn_aux<Ncnn):
                    ...
                    try:
                        #print(cnn_path)
                        m.weights = tf.Variable(tf.convert_to_tensor(np.load(cnn_path+'-W.npy'), np.float32), name = 'weights')
                        m.biases = tf.Variable(tf.convert_to_tensor(np.load(cnn_path+'-B.npy'), np.float32), name = 'biases')
                        Ncnn_aux+=1
                ...
                if(Nfc_aux<Nfc and Ncnn_aux==Ncnn):
                    ...
                    try:
                        #print(fc_path)
                        m.weights = tf.Variable(tf.convert_to_tensor(np.load(fc_path+'-W.npy'), np.float32), name = 'weights')
                        m.biases = tf.Variable(tf.convert_to_tensor(np.load(fc_path+'-B.npy'), np.float32), name = 'biases')
                        Nfc_aux+=1
                    except:
