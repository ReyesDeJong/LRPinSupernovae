...
class Linear(Module):
    ...
    def __init__(self, output_dim, batch_size=None, input_dim = None, act = 'linear',
                 keep_prob=tf.constant(1.0), weights_init= tf.truncated_normal_initializer(stddev=0.05),
                 bias_init= tf.constant_initializer(0.05), name="linear", param_dir=None, use_dropout = False, init_DH=False):
        self.name = name
        ...
        with tf.name_scope(self.name):
            ...
            with tf.name_scope(self.name+'_params'):
                if self.param_dir==None:
                    ...
                    self.biases = variables.biases(self.output_dim, initializer=self.bias_init, name='biases')
...
#9/10 dropout commented
    ...
    def _simple_lrp(self, R):
        ...
        stabilizer = 1e-9*(tf.where(tf.greater_equal(Zs,0), tf.ones_like(Zs, dtype=tf.float32), tf.ones_like(Zs, dtype=tf.float32)*-1))
        Zs += stabilizer
...
#   Stab added!!!!!!!!!!!!!! 
    def _alphabeta_lrp(self,R,alpha):
	    ...
	    #stab added
            epsilon=1e-16
            stabilizer = epsilon*(tf.where(tf.greater_equal(Zsp,0), tf.ones_like(Zsp, dtype=tf.float32), tf.ones_like(Zsp, dtype=tf.float32)*-1))
            Zsp += stabilizer
