...
#deje la cagaaaaaaa!!!!!111111   
    def forward(self,X):
        ''' forward passes the input data X to the layer's output neurons '''
        #with tf.name_scope(self.name+'_fwd_pass'):
        #        outX = X
    ...
    def lrp(self,R, lrp_var=None,param=None):
        ...
        if lrp_var is None or lrp_var.lower() == 'none' or lrp_var.lower() == 'simple':
            ...
            with tf.name_scope(self.name+'_alphabeta_relevance'):
                Rx = self._alphabeta_lrp(R,param)
            tf.summary.histogram(self.name, Rx)
            return Rx
        else:
    ...
    def lrpPy(self, sess, feed_dict, R, lrp_var=None, param=None):
        ...
        if lrp_var is None or lrp_var.lower() == 'none' or lrp_var.lower() == 'simple':
            #with tf.name_scope(self.name+'_simple_relevance'):
            Rx = self._simple_lrpPy(sess,feed_dict,R)
            #tf.summary.histogram(self.name, Rx)
            return Rx
        elif lrp_var.lower() == 'flat':
            return self._flat_lrpPy(sess,feed_dict,R)
        elif lrp_var.lower() == 'ww' or lrp_var.lower() == 'w^2':
            return self._ww_lrpPy(sess,feed_dict,R)
        elif lrp_var.lower() == 'epsilon':
            #with tf.name_scope(self.name+'_epsilon_relevance'):
            Rx = self._epsilon_lrpPy(sess,feed_dict,R,param)
            #tf.summary.histogram(self.name, Rx)
            return Rx
        elif lrp_var.lower() == 'alphabeta' or lrp_var.lower() == 'alpha':
            #with tf.name_scope(self.name+'_alphabeta_relevance'):
            Rx = self._alphabeta_lrpPy(sess,feed_dict,R,param)
            #tf.summary.histogram(self.name, Rx)
            return Rx
        else:
            print( 'Unknown lrp variant', lrp_var)
    ...
    def _simple_lrpPy(self,sess,feed_dict,R):
        raise NotImplementedError()

    def _flat_lrpPy(self,sess,feed_dict,R):
        raise NotImplementedError()

    def _ww_lrpPy(self,sess,feed_dict,R):
        raise NotImplementedError()

    def _epsilon_lrpPy(self,sess,feed_dict,R,param):
        raise NotImplementedError()

    def _alphabeta_lrpPy(self,sess,feed_dict,R,param):
        raise NotImplementedError()
