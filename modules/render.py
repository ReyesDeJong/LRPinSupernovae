...
plt.show()
#reproduction 
plt.imshow(R[:,:,0], cmap='jet', clim=(0,1))
plt.colorbar()
plt.show()
...
r=np.random.rand(28,28)*100

#r[:,:,0]+=-90
#r[:,:,2]+=90

#given a number of channel image relevances, normalize [0,1] through all of them
#and return a stack of [0,1] RGB images (one per channel relevance) to be plotted 

def plot_relevances_channel_norm(r):
    if len(r.shape)<3:
        r = r[...,np.newaxis]
    R=r
...
#%%
result = plot_relevances_channel_norm(r)
"""
